    ...
    try {
        ...
        var model = JSON.parse(properties.model);

        var shouldProceed = window.habitude.base.isNotEmpty(map) && window.habitude.base.isNotEmpty(model);
        ...
        if (!step) throw new Error(`[sync_payloads] Did not find action (${properties.action_id}) in map`);
